...
access_token = "EAAIe4hYZADdkBAJRbehZBhHfFL6cQ4j8BqaZAoQ00k4R1b9xdwoHWofeLtk6NFVwjompzAAddq28YhRspQnZBGdxLWxt3jZBlVwpTgwQCxe1QLmVDDVfI1igLWeJ9ZAQMYW59ull3DdrV8AnBuaLzAkpN4WtElH5zCPU0EjsZCGdpUy7UTfBZCgXfqO7JmQQuHzKfNaGGYT2HAZDZD"
php_url = "http://whatsappcrm.jespersoft.com/ajaxSendMessage"  #
...
def GetUserInput(jsonData) -> str:
    ...
    if jsonData["entry"][0]["changes"]:
        ...
        if changes["field"] and changes["field"] == "messages":
            try:
                getUserinput, user_number = messenger.get_message(
                    jsonData
                ), messenger.get_mobile(jsonData)
                user_name = messenger.get_name(jsonData)
            except:
                getUserinput, user_number = (
                    changes["value"]["messages"][0]["interactive"]["list_reply"]["id"],
            ...
            if "<user_name>" in botresponse["response"]:
                botresponse["response"] = botresponse["response"].replace(
            ...
            if botresponse["type"] == "button":
                r = messenger.send_button(
                    button=botresponse["response"],
                    recipient_id=user_number,
                )
            else:
                r = messenger.send_message(
            ...
            c = requests.post(
                ...
                data={
                    ...
                    "recipient_id": user_number,
                    "user_name": user_name,
                    "important": 1 if "demo" in str(getUserinput) else 0,
                    "flag": 1,
                },
            )
...
def chatHook(request):
    if request.method == "GET":
        ...
        if (
            ...
            return HttpResponse(
                request.GET.get("hub.challenge"), status=status.HTTP_200_OK
            )
        else:
...
def runMessageCampign(request):
    if request.method == "POST":
        getData = request.POST

        numbers = json.loads(getData["numbers"])
        message = json.loads(getData["message"])

        messenger = WhatsApp(access_token, phone_number_id=sender_id)
        for i in range(len(numbers)):
            r = messenger.send_button(
                recipient_id="91" + str(numbers[i]),
                button={
                    "header": message["message_header"],
                    "body": message["shortDescription"],
                    "footer": message["message_footer"],
                    "action": {
                        ...
                        "sections": [
                            {
                                ...
                                "rows": [
                                    ...
                                    {
                                        "id": "about Jesperapps",
                                        "title": "About Jesperapps",
                                        "description": "",
                                    },
                                    {
                                        "id": "Create Website / App",
